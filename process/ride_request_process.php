...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // Insert ride request
        $stmt = $pdo->prepare("INSERT INTO rides (customer_id, pickup_location, destination, emergency_type) VALUES (?, ?, ?, ?)");
        $stmt->execute([$_SESSION['user_id'], $pickup_location, $destination, $emergency_type]);
        
        $ride_id = $pdo->lastInsertId();
        
        $_SESSION['success'] = "Ride request submitted successfully! Redirecting to fare selection...";
        
        // Redirect to fare selection page
